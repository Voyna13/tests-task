...
class DepartmentsController extends Controller
{
    ...
    {
        $model = EmployeesDepartments::find()
            ->select('*, COUNT(*) as count')
            ->groupBy('employees_id')
            ->having('count < 2')
            ->andHaving('departments_id =' . $id)
            ->all();
        if(empty($model)){
            $this->findModel($id)->delete();
        }else{
            throw new NotFoundHttpException('Отдел не может быть удален, так как есть сотрудники закрепленные только за этим отделом.');
        }
